...
def assess_k_means(estimator, num_clusters, data, labels):
	t0 = time()
	estimator.fit(data)
...
for numc in range(1, true_k+1):
	km = KMeans(n_clusters=numc, init='k-means++', max_iter=100, n_init=5, verbose=False)
	assess_k_means(km, numc, X, labels)
